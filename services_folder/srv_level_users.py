...
async def on_xp_added(bot, guild_id: int, user_id: int):
    try:
        ...
        old_level = DB.level_users.get_user_level(guild_id, user_id)

        # если уровень изменился — сохраняем
        if new_level != old_level:
            DB.level_users.set_user_level(guild_id, user_id, new_level)
        ...
        rewards = DB.level_rewards.get_rewards(guild_id)  # список кортежей (уровень, role_id)
        ...
        if new_level > old_level:
            ch_id = DB.level_alerts.get_level_alerts_channel(guild_id)
            if not ch_id:
                return
...
def try_give_xp(bot,
    ...
    DB.level_users.add_xp(guild_id, user_id, random.randint(5, 10))
